...
def batch(tts_type,outname,params):
    ...
    # 根据模型yaml配置设置采样率
    if tts_type=='tts':
        torchaudio.save(tmp_dir + '/' + outname, audio_data, 22050, format="wav", backend='soundfile')   
    elif tts_type=='clone_eq':
        torchaudio.save(tmp_dir + '/' + outname, audio_data, 24000, format="wav", backend='soundfile')   
    else:
        torchaudio.save(tmp_dir + '/' + outname, audio_data, 24000, format="wav", backend='soundfile')    
